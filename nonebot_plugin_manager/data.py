...
class PluginList:
        ...
        for plugin in self.__plugin_list:
            if type == "ignore" or not self.__plugin_list[plugin]["ignore"]:
                if self.__plugin_list[plugin]["global"]:
                    if not type == "group" and user_id in self.__plugin_list[plugin]["user"]:
                        if self.__plugin_list[plugin]["user"][user_id]:
                            result[plugin]=True
                        else:
                            result[plugin]=False
                    else:
                        if not type == "user" and group_id in self.__plugin_list[plugin]["group"]:
    ...
    def block_plugin(self,
        ...
        for plugin in plugins:
            if plugin in self.__plugin_list and not self.__plugin_list[plugin]["ignore"] and self.__plugin_list[plugin]["global"]:
                result[plugin]=True
                if type == "global":
    ...
    def unblock_plugin(self,
        ...
        result = {}
        for plugin in plugins:
            if plugin in self.__plugin_list and not self.__plugin_list[plugin]["ignore"] and (type=="group" or self.__plugin_list[plugin]["global"]):
                result[plugin]=True
                if type == "global":
                    self.__plugin_list[plugin]=False
                elif type == "user" or user_id:
    ...
    def __ignore_plugin(self,plugins:Dict[str,bool]) -> "PluginList":
        for plugin in plugins:
            if plugin in self.__plugin_list:
                ...
                self.__plugin_list[plugin]={"ignore":plugins[plugin],"global":True,"user":{},"group":{}}
        return self 
    ...
    def __unignore_plugin(self,plugins:Dict[str,bool]) -> "PluginList":
        for plugin in plugins:
            if plugin in self.__plugin_list:
                self.__plugin_list[plugin]["ignore"]=False
            else:
                self.__plugin_list[plugin]={"ignore":plugins[plugin],"global":True,"user":{},"group":{}}
        return self

    # 添加插件（暂时不会用到）
    def __add_plugin(self, plugins: List[str],) -> "PluginList":
        for plugin in plugins:
    ...
    # 移除插件（暂时不会用到）
    def __remove_plugin(self, plugins: List[str],) -> "PluginList":
        for plugin in plugins:
            if plugin in self.__plugin_list:
